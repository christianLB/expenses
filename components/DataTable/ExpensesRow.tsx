...
const ExpensesRow = forwardRef<HTMLTableRowElement, ExpensesRowProps>((props, ref) => {
  ...
  const { selected: selectedCategory, SelectComponent: CategorySelect } =
    useSelect({ options: categories, placeHolder: "category", className: 'border-none' });
  ...
  return (
    <tr ref={dragDropRef} className={styles.expensesRow}>
      <td colSpan={2}className={`${styles.cell} py-0 border-none`} style={colorStyle}>{GroupsSelect}</td>
      <td colSpan={2} className={`${styles.cell} py-0 border-none`} style={colorStyle}>{CategorySelect}</td>
      <td className={`${styles.cell} py-0 border-none`} style={colorStyle}>{`${expenseDate.getDate()+1}/${expenseDate.getMonth()+1}`}</td>
      <td colSpan={8} className={`${styles.cell} py-0 border-none ${styles.paddingLeft}`} style={colorStyle}>{expense.name}</td>
